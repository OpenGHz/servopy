...
控制循环里有三个不同的量：**目标**是你希望到达的位置或速度；**反馈**是设备实际测到的关节状态；**参考**是 Servo 为下一周期生成的可执行轨迹。调参和排障时先区分这三个量。
...
`Servo.step()` 计算到参考生成这一步；应用负责读取设备与执行输出。`sample_reference(t)` 提供上一成功区间的插值。理想回放可以把参考当作下一反馈，真实设备必须始终使用测量值。

## 选择输入类型

| 你手里的目标 | 输入 | 内核如何处理 |
|---|---|---|
| 每个关节的位置 | `JointPositionCommand` | 关节位置反馈律；可选 Ruckig 直接规划位置目标 |

...
**目标：** 把 ServoCore 嵌入自己的 C++ 实时伺服循环，每周期传入最新目标和实际反馈并执行输出，无需 Python。需要支持 **C++17 或更新标准** 的编译器及标准库、**CMake 3.20 或更新版本**，以及 **Eigen 3.4 或更新的兼容版本**。

应用负责线程调度、单调时钟和设备 I/O；内核逐周期计算参考。当前 C++ 实现仍有动态分配，不单独保证硬实时 deadline。控制分层见[实时伺服控制](realtime-servo.md)，时序和停止要求见[执行契约](design.md)。

...
**目标：** 在实时伺服循环中对持续更新的目标施加 jerk 约束，并正确采样每个控制区间。前置：完成[实时伺服控制](realtime-servo.md)和[关节控制](joint-position.md)，安装 `.[ruckig]`。

Ruckig 在每次控制更新时根据当前参考状态和最新目标计算连续轨迹；Servo 执行本周期的区间，下一周期可用新目标再次更新。平滑是实时控制链中的可选运动生成后端。
